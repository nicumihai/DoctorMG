...
//		FileWriter fw=new FileWriter(patients);
		...
		BufferedWriter out = new BufferedWriter(new FileWriter("FilePatients.txt",true));
//		PrintWriter out=new PrintWriter(fw);
...
//		}
		out.newLine();
		out.write(p.getSSN() + "," + p.getName() + "," + p.getAddress());
		out.close();
	}
	...
	public void saveConsultationToFile(Consultation c) throws IOException		// save to file
	{
		...
		in.close(); // append
		*/
		BufferedWriter out = new BufferedWriter(new FileWriter("FileConsultations.txt",true));
